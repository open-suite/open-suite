...
Phase 2 live validation 2026-07-08: ran a full `deploy.sh` of main on the box — the exact clean-redeploy that used to reintroduce bugs — exit 0, all 17 authenticated smoke checks green. 2.1 confirmed: keycloak StatefulSet mounts the theme from the chart (no kubectl patch). 2.2 confirmed: element-web runs `ghcr.io/open-suite/element-web:v1.12.21`. Gotcha found and fixed: helm's 3-way merge cannot remove an imperatively-added container, so the old step-11 `patch-element-web-app` initContainer lingered on the in-place-upgraded element deploy and shadowed /app with an upstream-patched copy, bypassing the 2.2 image; deleted+recreated element-web from the chart to clear it (a from-scratch/clean-k3s deploy never has it). 2.3 Meet root cause pinned: `meet.<domain>/` routes to the meet-frontend SPA image, which serves its own index.html with no header; the meet-static-files injection (09 patch_static) lands on meet-static-nginx which only serves the unused `static-meet.<domain>` host — so Meet has never shown the shared nav. Element's header, by contrast, is a chart initContainer fed by a configmap and survives re-apply (smoke asserts it). Fixing Meet needs the header baked into the meet-frontend image or a chart initContainer + configmap like element's — its own PR, deferred. auth-gate still applied on floating `:main` by step 12 (2.4 will pin + move to helmfile).
