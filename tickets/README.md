...
Phase 2 live validation 2026-07-08: ran a full `deploy.sh` of main on the box — the exact clean-redeploy that used to reintroduce bugs — exit 0, all 17 authenticated smoke checks green. 2.1 confirmed: keycloak StatefulSet mounts the theme from the chart (no kubectl patch). 2.2 confirmed: element-web runs `ghcr.io/open-suite/element-web:v1.12.21`. Gotcha found and fixed: helm's 3-way merge cannot remove an imperatively-added container, so the old step-11 `patch-element-web-app` initContainer lingered on the in-place-upgraded element deploy and shadowed /app with an upstream-patched copy, bypassing the 2.2 image; deleted+recreated element-web from the chart to clear it (a from-scratch/clean-k3s deploy never has it). 2.3 correction: an earlier note here claimed Meet had no nav — that was measured against the pre-redeploy meet-frontend pod. After the full redeploy rolled meet-frontend to the pinned `ghcr.io/open-suite/meet-frontend` image, `meet.<domain>/` renders the full Open Suite nav from its own SPA bundle (`/assets/index-*.js`) — verified in-browser. So both static SPAs get the header declaratively and survive re-apply: Meet baked into its image, Element via a chart initContainer + configmap. 2.3 is effectively satisfied. Loose end: 09's `patch_static mb-meet meet-static-files` writes the button into meet-static-nginx, which only serves the unused `static-meet.<domain>` host — redundant cruft to remove, not a functional gap. auth-gate still applied on floating `:main` by step 12 (2.4 will pin + move to helmfile).
