...
require_literal "${sync_hook}" 'dist/files-init-opensuite-tp1.js'
...
# new cache-safe entry pathname, and await the child hook for every caller.
...
require_literal "${template_picker_patch}" "const PATCHED_BUNDLE_SHA256 = 'f3140718058e3b5c75f3e1383d8c9cabadfde32eb52ae56f93117ffc0e7c2031';"
require_literal "${template_picker_patch}" 'e.default.mixins=[...(e.default.mixins||[]),{mounted:ir}]'
require_literal "${template_picker_patch}" 'if(await ar'
require_literal "${template_picker_patch}" "const NEW_CONTROLLER = \"\\t\\tUtil::addInitScript('files', 'init-opensuite-tp1');\";"
require_literal "${template_picker_test}" 'NC34 immediate wrapper reproduces the lazy child open race'
require_literal "${template_picker_test}" 'candidate shares child-mounted readiness across concurrent callers'
require_literal "${template_picker_vue_test}" 'module.default.mixins = ['
require_literal "${template_picker_vue_test}" 'await pickerReady;'
