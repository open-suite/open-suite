...
- `patch-template-picker.php` — verifies the exact Nextcloud 34.0.0 Files
  ...
  named Files init bundle that caches and awaits the TemplatePicker import
  before synchronously constructing its Vue wrapper. The startup hook copies
  that bundle, its matching corrected source map, and its exact controller
  registration onto existing same-version PVCs before Apache starts. The
  renamed URL avoids reusing the immutable upstream asset. This is pinned
  Nextcloud 34 production behavior and must be reverified on image upgrades.
